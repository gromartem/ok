import { KEYBOARD_LETTERS, WORDS } from "./constns";
...
//Получаем слово и трансф в подчеркивания
...
//Создаем клаввиатуру и загруз на страницу
...
const checkLetter = (letter) => {
  ...
  hangmanImg.src = `imgs/hg-${10 -triesLeft}.png`;
...
//функция проигрыша,вывод сообщ. кнопка попробовать ещё раз.
...
const word = sessionStorage.getItem('word');
 ...
 if(status === 'win') {
  document.getElementById('hangman-img').src = 'imgs/hg-win.png'
...
//кнопка старт, рандомное слово, кол попыток,активация функций
  ...
  gameDiv.innerHTML +=
    '<p id="tries" class=" tries-text  mt-2">TRIES LEFT: <span id="tries-left" class="font-medium text-red-600">10</span></p>';
  ...
  gameDiv.appendChild(keyboardDiv);
   ...
   document.getElementById('quit').onclick = () => {
    const inSure = confirm('are you sure you want to quit and lose progress?');
    if(inSure){
      stopGame('quit');
    }
